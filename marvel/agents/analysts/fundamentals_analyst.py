...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "\n- `get_balance_sheet(ticker, curr_date)`：资产负债表详细数据。**curr_date 必须传当前分析日期**——财报期可能结束于分析日之前却公布于分析日之后，数据层靠它裁掉尚未公布的报告期"
            "\n- `get_cashflow(ticker, curr_date)`：现金流量表详细数据。**curr_date 必须传当前分析日期**（同上）"
            "\n- `get_income_statement(ticker, curr_date)`：利润表详细数据。**curr_date 必须传当前分析日期**（同上）"
