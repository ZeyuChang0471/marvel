...
### Fixed — 数据层（第二轮：还是「静默给出貌似合理的错误结果」）

- **行业榜真的包含跌幅榜**（`get_industry_comparison`）：接口按 `po=1` 降序返回全部约 86 个
  行业（`pz=100` 本来就取得回），但循环打到 `2*top_n` 就 `break` 并写上
  `... (showing top/bottom 20)`——实际只有**涨幅前 40**，跌幅榜一个都没有，跌幅行业在
  智能体眼里等于不存在。现在本地解析 + 按涨跌幅排序，头尾各取 N、排名用真实名次；
  `f3` 为 `"-"`（停牌）的行业不再被当成 0 塞进榜单中间；行业数少于 2N 时不会同一行
  既算涨幅榜又算跌幅榜。
- **东财请求失败不再伪装成「没有数据」**（`_em_get`）：429 / 5xx / 超时 / 连接错误原先只发
  一次请求，调用方拿到失败响应后 `.json()` 解析失败（或取到空 `data`），最终在报告里
  显示成「未上龙虎榜」「行业数据获取为空」——把**接口失败**说成了**真的没有数据**，而
  这两件事对模型的含义正好相反。现在退避 1/2/4s 重试 3 次（`EM_MAX_RETRIES` /
  `EM_RETRY_BASE_S` 可调），仍失败则**抛出**，由调用方已有的 except 分支输出「查询失败」。
- **mootdx 的单条 TCP 连接不再被多线程共用**（`_MOOTDX_LOCK`）：client 是模块级单例，
  选服务器与取数都没有锁，而通达信协议是有状态的请求-响应——两个线程同时用同一条连接
  会让响应错位。Web UI 的 tracker 是 per-session 的，两个浏览器会话可以各跑一轮分析，
  名称映射解析（`_build_name_code_map`）也走同一条连接。现在「选服务器 + 取数」整段
  串行；用可重入锁是因为 `_mootdx_call` 持锁期间会调 `reset_mootdx_client()`。
- **缓存改为原子写入**（新增 `dataflows/utils.py:atomic_write_text`）：名称映射 JSON、
  K 线 CSV、北向历史 CSV、yfinance OHLCV CSV 原先都是就地覆盖写，进程中途被杀/断电会
  留下一个"语法上能读、内容却残缺"的文件。北向那份每次重写**全部历史**，半截文件等于
  把过去攒下的所有交易日一起丢掉。现在统一「同目录临时文件 + `os.replace`」：失败时
  旧文件保持完整、临时文件被清掉。
- **800 根 K 线的截断被明说出来**（`get_stock_data`）：mootdx 单次只提供最近 800 根
  （约 3 年），请求更早的 `start_date` 时原先只在表头写 `# Total records: N`，等于把
  「我只取了近三年」说成「这只票三年以前没有数据」——模型据此算长周期均线、判断历史
  位置，报告里完全看不出缺了一段。现在检测到「取满 800 根且请求起点早于最早一根」时，
  在表头写明被截断的区间；`_load_ohlcv_astock`（指标路径，数值里夹带不了说明）至少
  留下 warning 日志。
- **当日 K 线缓存不再把盘中的半截数据当最终收盘价**（`_load_ohlcv_astock`）：原判据只有
  「缓存文件 mtime 是今天」，而且用的是**主机本地时区**的今天。盘中 10:30 跑过一次时
  写下的是当天那根**没走完**的 K 线；同一天晚上再跑，缓存照样命中，那根半截 K 线就被
  当成当日最终收盘价，均线/涨跌幅全偏而报告里毫无异常。现在按市场时区 + 交易日历判断：
  不是今天写的 → 重取；缓存里还没有今天那根 → 盘前/盘中/非交易日复用、已收盘则重取一次
  补上今天；缓存里已经有今天那根 → 只有**收盘之后**写下的才可能是完整的最终日线。
